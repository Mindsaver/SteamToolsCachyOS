...
- **Compatibility tools** — download **GE-Proton** ([GloriousEggroll/proton-ge-custom](https://github.com/GloriousEggroll/proton-ge-custom)) or **Proton-CachyOS** ([CachyOS/proton-cachyos](https://github.com/CachyOS/proton-cachyos)) into Steam’s `compatibilitytools.d`. Installed **Latest-line** builds show **Auto update** (and a **cog** for Proton-CachyOS SLR/arch) on that list row; fixed-version installs do not. Throttled GitHub checks run in the main process shortly after startup when auto update is enabled for that row. Optional **silent auto-install** (Settings), open tool folders or `user_settings.py`. After installing a new build, **restart Steam** so it rescans tools. **`x86_64_v4`** CachyOS archives are experimental; prefer **`x86_64`** unless you know you want v4.
