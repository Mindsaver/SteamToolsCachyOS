...
export async function simulateSymlinkStream(
  ...
  const hubRoot = '/home/arch/SteamToolsCachyOS'
  for (let i = 0; i < MOCK_GAMES.length; i++) {
    const g = MOCK_GAMES[i]
    const safeDir = `${hubRoot}/${g.name.replace(/[^a-zA-Z0-9 _\-()]/g, '').trim()}`
    await emit({
      type: 'progress',
      message: `[${i + 1}/${MOCK_GAMES.length}] ${g.name}`,
      current: i + 1,
      total: MOCK_GAMES.length,
    }, 60)
    await emit({ type: 'log', message: `  folder  ${safeDir}` }, 30)
    await emit({ type: 'log', message: `  link    common → ${g.installPath}` }, 30)
    if (g.compatDataPath) {
      await emit({ type: 'log', message: `  link    compatdata_prefix → ${g.compatDataPath}` }, 30)
    }
    if (g.system32Path) {
      await emit({ type: 'log', message: `  link    compatdata_windows_system32 → ${g.system32Path}` }, 30)
    }
    await emit({ type: 'log', message: `  file    Start in Steam.desktop` }, 30)
    await emit({ type: 'log', message: '' }, 20)
  }
...
export async function simulateFsrStream(cb: (p: SymlinkProgress) => void): Promise<void> {
  ...
  const srcDll = '/home/arch/Downloads/amdxcffx64.dll'

  await emit({ type: 'log', message: `Source: ${srcDll}` }, 80)
  await emit({ type: 'log', message: `Targets: ${targets.length} game prefixes` }, 80)
  await emit({ type: 'log', message: '' }, 40)

  for (let i = 0; i < targets.length; i++) {
    const g = targets[i]
    const dest = `${g.system32Path}/amdxcffx64.dll`
    await emit({
      type: 'progress',
      message: `[${i + 1}/${targets.length}] ${g.name}\n    → ${dest}`,
      current: i + 1,
  ...
  await emit({ type: 'log', message: '' }, 20)
  await emit({ type: 'done', message: `Done — copied to ${targets.length} prefixes.`, exitCode: 0 }, 0)
}
